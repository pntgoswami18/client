...
const Dashboard = () => {
  ...
  const fetchAllReports = async () => {
    try {
      ...
      const overdueInvoices = Array.isArray(reminders.data?.overdue_invoices)
        ? reminders.data.overdue_invoices
        : [];
  ...
  const fetchCardPrefs = async () => {
    try {
      ...
      setCardPrefs({
        ...
        show_unpaid_members_this_month:
          String(res.data.show_card_unpaid_members_this_month) !== 'false',
  ...
  const handleQuickUnlock = async (device) => {
    ...
    try {
      ...
      const reason = askUnlockReason
        ? unlockReason || 'Quick unlock from dashboard'
        : 'Quick unlock from dashboard';
        ...
        return (
          <div
            ...
            style={{
              ...
              minWidth: 0,
              boxShadow:
                hoveredCard === config.hoverIndex
                  ? '0 16px 40px rgba(0,0,0,0.35)'
                  : '0 8px 32px rgba(0,0,0,0.25)',
              border: '1px solid rgba(255,255,255,0.1)',
              transform: hoveredCard === config.hoverIndex ? 'translateY(-2px)' : 'translateY(0)',
              backgroundImage:
                'linear-gradient(to top, rgba(255,255,255,0.24), rgba(255,255,255,0) 60%), linear-gradient(135deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.35) 50%, rgba(255,255,255,0) 100%)',
              backgroundSize: '250% 250%',
              backgroundPosition:
                hoveredCard === config.hoverIndex ? 'right center' : 'left center',
              transition: 'box-shadow .3s ease, transform .3s ease, background-position .7s ease',
            }}
          >
            <h3 style={{ margin: 0, color: '#fff' }}>{config.title}</h3>
            <p style={{ fontSize: '2em', margin: 0, fontWeight: 'bold', alignSelf: 'flex-start' }}>
              {config.value}
            </p>
          </div>
        );
  ...
  const displayValue = (value, isCurrency = false) => {
    ...
    if (isNaN(num)) {
      return isCurrency ? formatCurrency(0, currency) : 0;
    }
    ...
    <div>
      ...
      <div
        style={{
          ...
          alignItems: 'stretch',
          overflowX: 'hidden',
          marginTop: birthdaysToday.length > 0 || paymentReminders.length > 0 ? '20px' : '0',
        }}
      >
        {renderCardsInOrder()}
      </div>
      ...
      {showBirthdays && birthdaysToday.length > 0 && (
        <div
          style={{
            position: 'fixed',
            top: 80,
            right: showPaymentReminders ? 440 : 24,
            zIndex: 1300,
            maxWidth: 360,
          }}
        >
            ...
            <div style={{ maxHeight: 400, overflowY: 'auto', padding: '10px 14px' }}>
              {paymentReminders.length > 0 && (
                <div style={{ marginBottom: '15px' }}>
                  <div style={{ fontWeight: 600, marginBottom: '10px', color: '#fca5a5' }}>
                    Overdue Payments
                  </div>
                  {paymentReminders.map((reminder) => {
                    ...
                    const whatsappUrl = reminder.phone
                      ? `https://wa.me/${encodeURIComponent(reminder.phone.replace(/\D/g, ''))}?text=${encodeURIComponent(whatsappMessage)}`
                      : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ opacity: 0.9, fontSize: '12px', margin: '4px 0' }}>
                          {formatCurrency(reminder.amount, currency)} • Due:{' '}
                          {new Date(reminder.due_date).toLocaleDateString()} •{' '}
                          {Math.floor(reminder.days_overdue)} days overdue
                        </div>
