...
const Member = () => {
  ...
  const fetchMembers = useCallback(
    async (page = currentPage, limit = itemsPerPage, search = searchTerm, filterType = filter) => {
      try {
  ...
  // Keep filter in sync when navigating with ?filter= (e.g. from Dashboard cards)
  useEffect(() => {
    const urlFilter = new URLSearchParams(location.search).get('filter') || 'all';
    setFilter((prev) => {
      if (prev === urlFilter) return prev;
      setCurrentPage(1);
      return urlFilter;
    });
  }, [location.search]);
  ...
  const calculateDueDate = useCallback(() => {
    ...
    if (selectedPlan.duration_days === 30) {
      ...
      calculatedDueDate.setDate(
        calculatedDueDate.getDate() + parseInt(selectedPlan.duration_days, 10)
      );
    }
  ...
  const handleEnrollmentWebSocketMessage = useCallback(
    (data) => {
      ...
      if (data.type === 'enrollment_started') {
        ...
        setBiometricError(
          `📱 WhatsApp welcome message failed for ${data.memberName}: ${data.error}`
        );
      } else if (data.type === 'whatsapp_welcome_error') {
        setBiometricError(
          `📱 WhatsApp welcome message error for member ${data.memberId}: ${data.error}`
        );
      }
    },
    [editingMember]
  ...
  const fetchCurrency = async () => {
    try {
      ...
      const referralEnabled =
        response.data.referral_system_enabled === 'true' ||
        response.data.referral_system_enabled === true;
  ...
  const addMember = async (e) => {
    ...
    try {
      ...
      if (res?.data?.id && photoFile) {
        ...
        const up = await axios.post(`/api/members/${res.data.id}/photo`, form, {
      ...
      if (res && res.data && res.data.id) {
        ...
        const selectedPlan =
          plans.find((p) => String(p.id) === String(membershipPlanId)) || plans[0];
      ...
      const msg =
        ...
        'Failed to add member. Please check the details and try again.';
      setAddError(msg);
    }
  };
  ...
  const updateMember = async (e) => {
    ...
    try {
      ...
      const body = {
        ...
        photo_url: photoUrl || null,
        is_admin: isAdmin,
        membership_plan_id: isAdmin
          ? null
          : membershipPlanId
            ? parseInt(membershipPlanId, 10)
            : null,
      };
      ...
      if (photoFile) {
        ...
        const up = await axios.post(`/api/members/${editingMember.id}/photo`, form, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        setPhotoUrl(up?.data?.photo_url || '');
      }
      ...
      const msg =
        error?.response?.data?.message ||
        'Failed to update member. Please check the details and try again.';
      setEditError(msg);
    }
  };
  ...
  const startEnrollment = async () => {
    ...
    try {
      ...
      if (!statusResponse.data.success || !statusResponse.data.data?.biometricServiceAvailable) {
        throw new Error(
          'Biometric service is not available. Please check if biometric integration is enabled.'
        );
      }
      ...
      if (devicesResponse.data.success && devicesResponse.data.devices.length > 0) {
        ...
        const onlineDevice = devicesResponse.data.devices.find(
  ...
  const deleteEnrollment = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete the biometric enrollment for ${editingMember.name}?`
      )
    ) {
      return;
    }
  ...
  const reEnroll = async () => {
    ...
    try {
      ...
      const deleteResponse = await axios.delete(
        `/api/biometric/members/${editingMember.id}/biometric`
      );
  ...
  const cancelEnrollment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setBiometricSuccess(
          `⏹️ Enrollment cancelled for ${ongoingEnrollment.memberName}. You can retry enrollment anytime.`
        );
      } else {
        setBiometricSuccess(
          `⏹️ Enrollment cancelled for ${ongoingEnrollment.memberName} (local cancellation). You can retry enrollment anytime.`
        );
      }
  ...
  const onCropComplete = async () => {
    ...
    try {
      const croppedImageData = await getCroppedImg(cropImageRef.current, crop, 'cropped-photo.jpg');
  ...
  return (
    <div>
      <Typography
        variant="h4"
        gutterBottom
        sx={{ borderBottom: '2px solid var(--accent-secondary-color)', pb: 1 }}
      >
        Members
      </Typography>
      <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
        <FormControl size="small" sx={{ minWidth: 220 }}>
          ...
          <Select label="Filter" value={filter} onChange={handleFilterChange}>
            ...
            <MenuItem value="everyone">All members</MenuItem>
        ...
        >
          ...
          >
            ...
            <DialogActions sx={{ px: 0 }}>
              ...
              <Button type="submit" variant="contained" disabled={plans.length === 0 && !isAdmin}>
                Add Member
              </Button>
            </DialogActions>
          </Box>
        ...
        >
          ...
          >
            ...
            {editingMember && editingMember.is_admin === 1 && !isAdmin && (
              ...
              >
                ℹ️ Changing this member from admin to regular member will allow them to select a
                membership plan
              </Typography>
            )}
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (p) {
                    setInvoiceAmount(String(p.price));
                  }
                }}
              >
                {plans.map((p) => (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {members.map((member) => {
              ...
              const isBirthdayToday =
                ...
                member.birthday.slice(5, 10) === `${todayMonth}-${todayDay}`;
              const whatsappHref = member.phone
                ? `https://wa.me/${encodeURIComponent(member.phone.replace(/\D/g, ''))}?text=${encodeURIComponent(`Happy Birthday, ${member.name}! 🎉🎂 Wishing you a fantastic year ahead from ${currency} Gym!`)}`
                : null;
              return (
                <Card
                  key={member.id}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {member.phone || 'No phone'} • {member.birthday || 'No birthday'} •
                            Joined: {formatDateToLocalString(new Date(member.join_date))}
                          </Typography>
                        </Box>
                      </Box>
      ...
      >
        <DialogTitle>Biometric data for {editingMember?.name || 'Member'}</DialogTitle>
        <DialogContent>
          {biometricError && (
            <Alert severity="error" sx={{ mb: 2 }}>
              {biometricError}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 1 }}>
              ...
              >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <Chip label="Biometric Service" color="primary" size="small" variant="outlined" />
                  <Typography variant="body2" color="text.secondary">
                ...
                <TextField
                  ...
                  InputProps={{
                    ...
                    sx: {
                      ...
                      '& .MuiInputBase-input': {
                        color: memberBiometricStatus.biometricId
                          ? 'text.primary'
                          : 'text.secondary',
                      },
                    },
                ...
                {memberBiometricStatus.hasFingerprint ? (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Button
                      ...
                      startIcon={
                        isBiometricButtonLoading('delete') ? (
                    ...
                    <Button
                      ...
                      startIcon={
                        isBiometricButtonLoading('reenroll') ? (
                          <CircularProgress size={20} />
                        ) : (
                          <RefreshIcon />
                        )
                  ...
                  <Button
                    ...
                    startIcon={
                      isBiometricButtonLoading('enroll') ? (
                        <CircularProgress size={20} />
                      ) : (
                        <FingerprintIcon />
                      )
                ...
                {ongoingEnrollment && (
                  <Box sx={{ mt: 2 }}>
                    <Button
                      ...
                      startIcon={
                        isBiometricButtonLoading('cancel') ? (
                          <CircularProgress size={20} />
                        ) : (
                          <CancelIcon />
                        )
      ...
      >
        ...
        <DialogContent>
          <Box
            sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, mt: 1 }}
          >
            {cameraLoading ? (
              <Box
                sx={{
              ...
              <>
                <video
                  ...
                  onError={(e) => {
                    ...
                    setCameraError(
                      'Video playback error: ' + (e.target.error?.message || 'Unknown error')
                    );
                    setVideoReady(false);
                  }}
                  onStalled={() => {
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={applyCrop} variant="contained" disabled={!croppedImageUrl}>
            Apply Crop
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        >
          <Typography variant="h6">{memberDetails?.member?.name || 'Member Details'}</Typography>
                    ...
                    <CardContent sx={{ textAlign: 'center' }}>
                      <Avatar
                        ...
                        sx={{
                          ...
                          mb: 2,
                          bgcolor:
                            memberDetails.member.is_admin === 1 ? 'warning.main' : 'primary.main',
                        }}
                      >
                        {!memberDetails.member.photo_url &&
                          (memberDetails.member.name || '').slice(0, 2).toUpperCase()}
                      </Avatar>
                      ...
                      <Grid container spacing={3} sx={{ width: '100%' }}>
                        <Grid item xs={12} sm={6} md={3}>
                          <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 3 }}>
                            <PhoneIcon
                              sx={{ mr: 2, color: 'text.secondary', mt: 0.5, flexShrink: 0 }}
                            />
                            <Box sx={{ flex: 1 }}>
                              <Typography variant="body2" color="text.secondary" gutterBottom>
                                Phone Number
                        ...
                        <Grid item xs={12} sm={6} md={3}>
                          <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 3 }}>
                            ...
                            <Box sx={{ flex: 1 }}>
                              ...
                              <Typography variant="body1" sx={{ fontWeight: 500 }}>
                                {memberDetails.member.birthday
                                  ? formatDateToLocalString(new Date(memberDetails.member.birthday))
                                  : 'Not provided'}
                              </Typography>
                            </Box>
                          </Box>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <Typography variant="body1" gutterBottom>
                          You have an unused referral discount of{' '}
                          {formatCurrency(
                            memberDetails.unusedReferralDiscount.discount_amount,
                            currency
                          )}
                  ...
                  <Grid item xs={12} sm={6}>
                    <Card>
                      <CardHeader
                        ...
                        avatar={
                          <Chip
                            ...
                            color={
                              memberDetails.latestInvoice.status === 'paid'
                                ? 'success'
                      ...
                      <CardContent>
                        ...
                        <Typography variant="body2" color="text.secondary" gutterBottom>
                          Due Date:{' '}
                          {formatDateToLocalString(new Date(memberDetails.latestInvoice.due_date))}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
        ...
        <DialogActions sx={{ justifyContent: 'space-between', px: 3, py: 2 }}>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Button
              ...
              startIcon={
                currentViewingMember?.is_active === 0 ? <ToggleOnIcon /> : <ToggleOffIcon />
              }
      ...
      >
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
            ...
            {currentViewingMember?.is_active === 0 && (
              <Alert severity="info">
                This member is currently deactivated. Recording this payment will automatically
                activate them.
              </Alert>
            )}
          </Box>
        ...
        <DialogActions>
          <Button onClick={() => setOpenPaymentDialog(false)} disabled={paymentLoading}>
            Cancel
          </Button>
          <Button
