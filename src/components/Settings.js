...
const GeneralSettings = ({ onUnsavedChanges, onSave }) => {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      const {
        ...
        secondary_color,
        primary_color_mode,
        secondary_color_mode,
        primary_color_gradient,
        secondary_color_gradient,
        payment_reminder_days_after_due,
      ...
      const normalizedReferralSystemEnabled =
        referral_system_enabled === 'true' || referral_system_enabled === true;
      const normalizedWhatsappWelcomeEnabled =
      ...
      if (primary_color_mode) {
        setPrimaryColorMode(primary_color_mode);
      }
      if (secondary_color_mode) {
        setSecondaryColorMode(secondary_color_mode);
      }
      if (primary_color_gradient !== undefined) {
        setPrimaryGradient(primary_color_gradient);
      }
      if (secondary_color_gradient !== undefined) {
        setSecondaryGradient(secondary_color_gradient);
      }
      if (payment_reminder_days_after_due) {
        setPaymentReminderDaysAfterDue(String(payment_reminder_days_after_due));
      }
      if (payment_grace_period_days !== undefined) {
        setPaymentGracePeriodDays(String(payment_grace_period_days));
      }
      if (morning_session_start) {
        setMorningStart(morning_session_start);
      }
      if (morning_session_end) {
        setMorningEnd(morning_session_end);
      }
      if (evening_session_start) {
        setEveningStart(evening_session_start);
      }
      if (evening_session_end) {
        setEveningEnd(evening_session_end);
      }
      ...
      setShowNewMembersThisMonth(
        show_card_new_members_this_month === 'true' || show_card_new_members_this_month === true
      );
      setShowUnpaidMembersThisMonth(
        show_card_unpaid_members_this_month === 'true' ||
          show_card_unpaid_members_this_month === true
      );
      setShowActiveSchedules(
        show_card_active_schedules === 'true' || show_card_active_schedules === true
      );

      if (ask_unlock_reason !== undefined) {
        setAskUnlockReason(normalizedAskUnlockReason);
      }
      if (referral_system_enabled !== undefined) {
        setReferralSystemEnabled(normalizedReferralSystemEnabled);
      }
      if (referral_discount_amount) {
        setReferralDiscountAmount(String(referral_discount_amount));
      }
      if (whatsapp_welcome_enabled !== undefined) {
        setWhatsappWelcomeEnabled(normalizedWhatsappWelcomeEnabled);
      }
      if (whatsapp_welcome_message) {
        setWhatsappWelcomeMessage(whatsapp_welcome_message);
      }
  ...
  const checkForChanges = useCallback(() => {
    ...
    const hasChanges =
      JSON.stringify(currentValues) !== JSON.stringify(initialValues.current) || logoFile !== null;
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto' }}>
      <form>
        ...
        <label htmlFor="gym-logo" style={{ display: 'block', marginTop: 8 }}>
          Gym Logo
        </label>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          {gymLogo ? (
            <img
              src={gymLogo}
              alt="logo"
              style={{ height: '64px', borderRadius: 6, border: '1px solid #eee' }}
            />
          ) : (
            <div
              style={{
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (file) {
                setLogoFile(file);
                try {
                  setGymLogo(URL.createObjectURL(file));
                } catch (_) {}
              }
            }}
          />
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <FormGroup sx={{ mt: 1 }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={askUnlockReason}
                onChange={(e) => setAskUnlockReason(e.target.checked)}
              />
            }
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <FormGroup sx={{ mt: 1 }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={referralSystemEnabled}
                onChange={(e) => setReferralSystemEnabled(e.target.checked)}
              />
            }
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <FormGroup sx={{ mt: 1 }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={whatsappWelcomeEnabled}
                onChange={(e) => setWhatsappWelcomeEnabled(e.target.checked)}
              />
            }
        ...
        <Typography variant="caption" display="block" sx={{ mt: 1 }}>
          When enabled, a WhatsApp message will be automatically sent to members when they complete
          biometric enrollment for the first time.
        </Typography>
      ...
      <div style={{ marginTop: '30px' }}>
        ...
        <FormGroup sx={{ mt: 1 }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={crossSessionCheckinRestriction}
                onChange={(e) => setCrossSessionCheckinRestriction(e.target.checked)}
              />
            }
...
const Settings = () => {
  ...
  return (
    <Box>
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs value={getActiveTab()} onChange={handleTabChange} aria-label="settings tabs">
          <Tab icon={<SettingsIcon />} label="General" sx={{ minHeight: 72 }} />
          <Tab icon={<DeviceHubIcon />} label="ESP32 Devices" sx={{ minHeight: 72 }} />
          <Tab icon={<MonitorIcon />} label="Device Monitor" sx={{ minHeight: 72 }} />
          <Tab icon={<AnalyticsIcon />} label="Device Analytics" sx={{ minHeight: 72 }} />
        </Tabs>
      </Paper>
      ...
      <Routes>
        <Route
          path="/"
          element={<GeneralSettings onUnsavedChanges={handleUnsavedChanges} onSave={handleSave} />}
        />
        <Route
          path="/esp32-devices"
          element={
            <ESP32DeviceManager onUnsavedChanges={handleUnsavedChanges} onSave={handleSave} />
          }
        />
        <Route
          path="/esp32-monitor"
          element={<ESP32Monitor onUnsavedChanges={handleUnsavedChanges} onSave={handleSave} />}
        />
        <Route
          path="/esp32-analytics"
          element={<ESP32Analytics onUnsavedChanges={handleUnsavedChanges} onSave={handleSave} />}
        />
      </Routes>
