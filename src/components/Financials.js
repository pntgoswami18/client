...
const Financials = () => {
  ...
  // Multi-select state
  const [selectedOutstanding, setSelectedOutstanding] = useState(new Set());
  ...
  const fetchFinancialSummary = useCallback(
    async (table = 'all', page = 1, limit = itemsPerPage, search = '') => {
      try {
        ...
        setFinancialSummary((prev) => ({
          ...
          ...def,
          ...d,
          outstandingInvoices: Array.isArray(d.outstandingInvoices)
            ? d.outstandingInvoices
            : prev.outstandingInvoices,
          paymentHistory: Array.isArray(d.paymentHistory) ? d.paymentHistory : prev.paymentHistory,
          memberPaymentStatus: Array.isArray(d.memberPaymentStatus)
            ? d.memberPaymentStatus
            : prev.memberPaymentStatus,
        }));
  ...
  const handleOutstandingPageChange = (event, page) => {
    ...
    debouncedFetchFinancialSummary('outstanding', page, itemsPerPage, outstandingSearch);
  };

  const handlePaymentsPageChange = (event, page) => {
    setPaymentsPage(page);
    setSelectedPayments(new Set());
    debouncedFetchFinancialSummary('payments', page, itemsPerPage, paymentsSearch);
  };
  ...
  const debouncedOutstandingSearch = useMemo(
    () =>
      debounce((value) => {
        setOutstandingPage(1);
        setSelectedOutstanding(new Set());
        fetchFinancialSummary('outstanding', 1, itemsPerPage, value);
      }, 400),
    [fetchFinancialSummary, itemsPerPage]
  );

  const debouncedPaymentsSearch = useMemo(
    () =>
      debounce((value) => {
        setPaymentsPage(1);
        setSelectedPayments(new Set());
        fetchFinancialSummary('payments', 1, itemsPerPage, value);
      }, 400),
    [fetchFinancialSummary, itemsPerPage]
  );

  const handleOutstandingSearchChange = (e) => {
    const value = e.target.value;
    setOutstandingSearch(value);
    debouncedOutstandingSearch(value);
  };

  const handlePaymentsSearchChange = (e) => {
    const value = e.target.value;
  ...
  const handleSelectAllOutstanding = (e) => {
    if (e.target.checked) {
      setSelectedOutstanding(new Set(financialSummary.outstandingInvoices.map((inv) => inv.id)));
    } else {
      setSelectedOutstanding(new Set());
    }
  };
  ...
  const handleSelectAllPayments = (e) => {
    if (e.target.checked) {
      setSelectedPayments(new Set(financialSummary.paymentHistory.map((p) => p.id)));
    } else {
      setSelectedPayments(new Set());
    }
  };

  const toggleOutstandingRow = (id) => {
  ...
  const handleBulkDeleteOutstanding = async () => {
    if (
      !window.confirm(
        `Delete ${selectedOutstanding.size} selected invoice(s)? This action cannot be undone.`
      )
    )
      return;
    ...
    try {
      await Promise.all(
        [...selectedOutstanding].map((id) => axios.delete(`/api/payments/invoices/${id}`))
      );
      setSelectedOutstanding(new Set());
      fetchFinancialSummary('outstanding', outstandingPage, itemsPerPage, outstandingSearch);
    } catch (error) {
      alert(error?.response?.data?.message || 'Error deleting some invoices.');
    } finally {
  ...
  const handleBulkDeletePayments = async () => {
    if (
      !window.confirm(
        `Delete ${selectedPayments.size} selected payment(s)? Associated invoices will revert to unpaid.`
      )
    )
      return;
    setBulkDeleting(true);
    try {
      await Promise.all([...selectedPayments].map((id) => axios.delete(`/api/payments/${id}`)));
      setSelectedPayments(new Set());
      fetchFinancialSummary('payments', paymentsPage, itemsPerPage, paymentsSearch);
      window.dispatchEvent(new CustomEvent('paymentDeleted'));
    } catch (error) {
      alert(error?.response?.data?.message || 'Error deleting some payments.');
    } finally {
      setBulkDeleting(false);
    }
  };
  ...
  const fetchMembers = async () => {
    try {
      ...
      const deactivatedMembersData = Array.isArray(deactivatedRes.data.members)
        ? deactivatedRes.data.members
        : [];
      ...
      const validMembers = allMembersData.filter(
    ...
    <div>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography
          variant="h4"
          gutterBottom
          sx={{ borderBottom: '2px solid var(--accent-secondary-color)', pb: 1, mb: 0 }}
        >
          Financials
        </Typography>
      ...
      <Box ref={outstandingRef} sx={{ marginBottom: '2rem' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Typography variant="body2" color="text.secondary">
              Showing {(outstandingPage - 1) * itemsPerPage + 1} to{' '}
              {Math.min(outstandingPage * itemsPerPage, paginationMeta.outstanding.total)} of{' '}
              {paginationMeta.outstanding.total} invoices
            </Typography>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1.5, flexWrap: 'wrap' }}>
          <TextField
            ...
            onChange={handleOutstandingSearchChange}
            sx={{ minWidth: 280 }}
            InputProps={{
              startAdornment: (
          ...
          />
          {selectedOutstanding.size > 0 && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Typography variant="body2" color="text.secondary">
                {selectedOutstanding.size} selected
              </Typography>
              <Tooltip title="Delete selected invoices">
                <Button
                  ...
                  onClick={handleBulkDeleteOutstanding}
              ...
              <Table>
                  ...
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selectedOutstanding.size > 0 &&
                          selectedOutstanding.size < financialSummary.outstandingInvoices.length
                        }
                        checked={
                          financialSummary.outstandingInvoices.length > 0 &&
                          selectedOutstanding.size === financialSummary.outstandingInvoices.length
                ...
                <TableBody>
                  {financialSummary.outstandingInvoices.map((invoice) => {
                    ...
                    const isSelected = selectedOutstanding.has(invoice.id);
                    return (
                      <TableRow
                        key={invoice.id}
                        selected={isSelected}
                        sx={{ cursor: 'pointer' }}
                        onClick={() => toggleOutstandingRow(invoice.id)}
                      >
                        <TableCell padding="checkbox">
      ...
      <Box sx={{ marginBottom: '2rem' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Showing {(paymentsPage - 1) * itemsPerPage + 1} to{' '}
            {Math.min(paymentsPage * itemsPerPage, paginationMeta.payments.total)} of{' '}
            {paginationMeta.payments.total} payments
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1.5, flexWrap: 'wrap' }}>
          <TextField
            size="small"
            placeholder="Search by member name, payment ID, or invoice ID"
          ...
          {selectedPayments.size > 0 && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Tooltip title="Delete selected payments (invoices will revert to unpaid)">
                <Button
                  size="small"
                  variant="outlined"
                  color="error"
        ...
        {loadingStates.payments ? (
                  ...
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selectedPayments.size > 0 &&
                          selectedPayments.size < financialSummary.paymentHistory.length
                        }
                        checked={
                          financialSummary.paymentHistory.length > 0 &&
                          selectedPayments.size === financialSummary.paymentHistory.length
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={async (e) => {
                                ...
                                if (
                                  window.confirm(
                                    `Are you sure you want to delete payment #${payment.id}? This will make the associated invoice unpaid.`
                                  )
                                ) {
                                  try {
                                    ...
                                    window.dispatchEvent(
                                      new CustomEvent('paymentDeleted', {
                                        detail: { paymentId: payment.id },
                                      })
                                    );
                                    alert(
                                      'Payment deleted successfully. Invoice status updated to unpaid.'
                                    );
                                  } catch (error) {
          ...
          <Box sx={{ p: 2, border: '1px dashed #ccc', borderRadius: 2, textAlign: 'center' }}>
            ...
            <Typography>
              {paymentsSearch ? 'No payments match your search.' : 'No payment history available.'}
            </Typography>
          </Box>
        )}
      </Box>
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Showing {(membersPage - 1) * itemsPerPage + 1} to{' '}
            {Math.min(membersPage * itemsPerPage, paginationMeta.members.total)} of{' '}
            {paginationMeta.members.total} members
          </Typography>
        </Box>
                      ...
                      <TableCell>
                        <span
                          style={{
                            ...
                            fontWeight: '500',
                            background:
                              member.last_invoice_status === 'paid'
                                ? 'rgba(34, 197, 94, 0.1)'
      ...
      <Box sx={{ marginBottom: '3rem' }}>
        ...
        <Card sx={{ display: 'none' }}>
          <CardContent>
            ...
            <Box
              ...
              sx={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '500px' }}
            >
              <TextField
                label="Plan Name"
                value={planName}
        ...
        <Typography variant="h6" sx={{ color: 'var(--accent-secondary-color)', mb: 1 }}>
          Existing Plans
        </Typography>
        {plans.length > 0 ? (
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1rem', mt: 1 }}>
            <Typography variant="caption" color="text.secondary" sx={{ mb: 1 }}>
              Note: Admin users are exempt from payments and cannot have payments recorded against
              them.
            </Typography>
            <Autocomplete
              ...
              onChange={async (_e, value) => {
                ...
                if (value && value.id) {
                  try {
                    const res = await axios.get(`/api/payments/unpaid`, {
                      params: { member_id: value.id },
                    });
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="Member"
                  placeholder="Type member name (includes deactivated)"
                />
              )}
            />
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
              ...
              <Button
                onClick={() =>
                  setManualInvoiceId(String(Math.floor(100000 + Math.random() * 900000)))
                }
              >
                Generate Invoice ID
              </Button>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    if (inv) {
                      setManualAmount(String(inv.amount));
                    }
                  }}
                >
                  {unpaidInvoices.map((inv) => {
                    ...
                    return (
                      <MenuItem key={inv.id} value={inv.id}>
                        #{inv.id} — {formatCurrency(inv.amount, currency)} — Due{' '}
            ...
            onClick={async () => {
              try {
                ...
                if (manualMember && manualMember.is_admin === 1) {
                  alert(
                    'Admin users are exempt from payments and cannot have payments recorded against them.'
                  );
                  return;
                }
                ...
                if (manualInvoiceId) {
                  payload.invoice_id = parseInt(manualInvoiceId, 10);
                }
                ...
                if (manualMember && manualMember.is_active === 0) {
                  try {
                    ...
                    alert(
                      'Payment recorded successfully, but there was an error reactivating the member. Please activate them manually.'
                    );
                  }
                }
                ...
                alert(
                  'Payment recorded successfully!' +
                    (manualMember && manualMember.is_active === 0
                      ? ' Member has been reactivated and can now access the door.'
                      : '')
                );
              } catch (e) {
                console.error(e);
      ...
      >
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1rem', mt: 1 }}>
            <Typography variant="caption" color="text.secondary" sx={{ mb: 1 }}>
              Note: Admin users are exempt from payments and cannot have invoices created against
              them.
            </Typography>
            ...
            <TextField
              ...
              type="date"
              value={invJoinDate}
              onChange={(e) => setInvJoinDate(e.target.value)}
              InputLabelProps={{ shrink: true }}
              helperText="Member's joining date for due date calculation"
            />
          </Box>
        </DialogContent>
        <DialogActions>
            ...
            onClick={async () => {
              try {
                ...
                if (selectedMember && selectedMember.is_admin === 1) {
                  setCreateInvoiceError(
                    'Admin users are exempt from payments and cannot have invoices created against them'
                  );
                  return;
                }
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1rem', mt: 1 }}>
            ...
            <SearchableMemberDropdown
              ...
              onChange={(e) => {
                setEditInvMemberId(e.target.value);
              }}
              members={nonAdminMembers}
              label="Member"
              placeholder="Search members by name, ID, or phone..."
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  const p = plans.find((pl) => String(pl.id) === String(e.target.value));
                  if (p) {
                    setEditInvAmount(String(p.price));
                  }
                }}
              >
            ...
            onClick={async () => {
              try {
                ...
                const selectedMember = members.find(
                  (m) => String(m.id) === String(editInvMemberId)
                );
                if (selectedMember && selectedMember.is_admin === 1) {
                  alert(
                    'Admin users are exempt from payments and cannot have invoices created against them.'
